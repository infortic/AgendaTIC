...
import com.agenda.repository.ProcedimentoRepository;
import com.agenda.repository.SalaRepository;

import io.swagger.annotations.Api;

@Api(value = "Serviços responsavel por receber e processar requisições de procedimentos")
@RestController
@RequestMapping("/procedimento")
public class ProcedimentoWS {
	@Autowired
	private ProcedimentoRepository dao;
	...
	public ResponseEntity<?> buscarPorNome(@PathVariable("nome") String nome) {
		List<ProcedimentoEntity> pessoa = dao.findByNomeIgnoreCaseContaining(nome);
		if (pessoa.isEmpty()) {
	...
	public ResponseEntity<?> buscarPorId(@PathVariable("id") long id) {
		Optional<ProcedimentoEntity> pessoa = dao.findById(id);
		if (pessoa.isPresent()) {
			return new ResponseEntity<>(pessoa, HttpStatus.OK);
		} else {
